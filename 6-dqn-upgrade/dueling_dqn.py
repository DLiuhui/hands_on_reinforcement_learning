...
class DuelingDQN:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, learning_rate, gamma, epsilon,
        ...
        self.q_net = VAnet(self.state_dim, hidden_dim, self.action_dim).to(device)
        # 目标网络
        self.target_q_net = VAnet(self.state_dim, hidden_dim, self.action_dim).to(device)
        # 优化器
    ...
    def update(self, transition_dict):
        ...
        # Dueling DQN 在计算 Q 值的逻辑同一般 DQN 相同, 只是将 Q 网络换成了 VA 网络
...
def main():
    ...
    plt.savefig("dueling-dqn-return.png")
    ...
    plt.ylabel('Q value')
    plt.title('Dueling DQN on {}'.format(env_name))
    plt.savefig("dueling-dqn-q-value.png")
